...
| `data-v_is-email-with-domain` | Similar to `data-v_is-email`, but also checks whether the email address is for the domain specified in the argument. | `<input type="text" data-v_is-email-with-domain="test.com">` | "user\@domain.com" | "alphabet soup @", "user\@wrongdomain.com" |
...
(Not enough customization for your application? You may want to check out [Just-validate](https://github.com/horprogs/Just-validate), a slightly more complex validator built on similar concepts.)
